...
class CDMenuPage(Page):
    ...
    def on_init(self):
        ...
        if self.info.force_wisqi:
            y += lh + sep
            self.main.wisqi_button = ui.FlatButton(self.main, x, y, bw, bh, _("Install inside Windows"))
            y += bh + 2
            ...
            txt = txt % dict(distro=distro_name)
            self.main.wisqi_label = ui.Label(self.main, x, y, lw, lh, txt)
            self.main.wisqi_button.on_click = self.on_wisqi
