...
import unittest
import sys, os
sys.path.insert(0, 'build\\wisqi\\lib')
from wisqi.backends.win32 import backend
from version import application_name, version, revision
from wisqi import application

import mock
from wisqi.backends.win32 import registry
class BackendTests(unittest.TestCase):
    def setUp(self):
        w = application.Wisqi(application_name, version,
                             revision, 'build\\wisqi')
        # To get things like info.locale set.
        w.parse_commandline_arguments()
        self.back = backend.WindowsBackend(w)
        w.info.original_exe = os.path.join(os.getcwd(), 'build', 'wisqi.exe')

        # Data
        self.uninstall_keys = [
            ('HKEY_LOCAL_MACHINE', 'registry-key', 'UninstallString',
             'Z:\\tmp\\uninstall-wisqi.exe'),
            ('HKEY_LOCAL_MACHINE', 'registry-key', 'InstallationDir', '/tmp'),
            ('HKEY_LOCAL_MACHINE', 'registry-key', 'DisplayName', 'Squibby'),
            ('HKEY_LOCAL_MACHINE', 'registry-key', 'DisplayIcon',
             os.path.join(os.getcwd(), 'build\\wisqi\\data\\images\\Wubi.ico')),
            ('HKEY_LOCAL_MACHINE', 'registry-key', 'DisplayVersion',
             self.back.info.version_revision),
            ('HKEY_LOCAL_MACHINE', 'registry-key', 'Publisher', 'Squibby'),
    ...
    def test_create_uninstaller(self):
        ...
        self.back.info.distro = self.back.parse_isolist(
                                    'build/wisqi/data/isolist.ini')[0]
        ...
        # TODO mktempd
        self.assert_(os.path.exists('/tmp/uninstall-wisqi.exe'),
            'Did not install uninstaller binary.')
        os.remove('/tmp/uninstall-wisqi.exe')
